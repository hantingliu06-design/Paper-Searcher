...
- **有评估但不过度宣传**：包含小规模人工标注测试集，披露模拟数据和样本规模限制。

建议先用 [演示脚本](docs/demo-guide.md) 录制一段短视频，再将项目推送到自己的 GitHub。源码、截图和报告可以直接展示；交互后端不能部署在纯静态 GitHub Pages 上。

GitHub 没有通用的项目评分机制，Star 和招聘评价也无法保证。这个项目的目标是让评审快速运行、理解并检查你的技术决策。发布前建议读懂评分、验证和预算控制，并加入一个你熟悉领域的真实案例。已完成和未验证的内容见 [验证记录](docs/validation.md)。
